...
def download_dlq_messages(queue_url: str, output_dir: str, delete_after_download: bool = False, 
                          region: str = 'us-east-2', max_messages: Optional[int] = None):
    """
...
Environment Variables:
  ...
  DLQ_QUEUE_URL       SQS DLQ URL (if not provided via --queue-url)
    ...
    parser.add_argument(
        '--queue-url',
        type=str,
        default=os.environ.get('DLQ_QUEUE_URL'),
        help='SQS Dead Letter Queue URL (or set DLQ_QUEUE_URL env var)'
    )
    parser.add_argument(
        '--output',
        type=str,
        default='DLQ',
        help='Output directory to save messages (default: ./DLQ)'
    )
    parser.add_argument(
        '--delete',
        action='store_true',
        help='Delete messages from queue after downloading'
    )
    parser.add_argument(
        '--region',
        type=str,
    ...
    if not args.queue_url:
        print("Error: --queue-url is required or set DLQ_QUEUE_URL environment variable")
        parser.print_help()
        sys.exit(1)
    ...
    print(f"Downloading DLQ messages...")
