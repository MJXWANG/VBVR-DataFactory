...
def load_messages_from_dlq(dlq_dir: str) -> List[Dict[str, Any]]:
    ...
    json_files = [f for f in dlq_path.glob('*.json') 
                  if not f.name.endswith('.bak') and not f.name.endswith('.original')]
...
def send_message_batch(sqs_client, queue_url: str, messages: List[Dict], 
                       batch_id: int, retry_count: int = 3) -> tuple:
    """
        ...
        try:
            ...
            if failed > 0:
                ...
                for failure in response.get('Failed', []):
                    print(f"  Failed ID {failure['Id']}: {failure.get('Code', 'Unknown')} - {failure.get('Message', 'No message')}")
...
def push_messages_to_sqs(dlq_dir: str, queue_url: str, region: str = 'us-east-2',
    ...
    for batch_idx, batch in enumerate(batches, 1):
        ...
        successful, failed, failed_msgs = send_message_batch(
            sqs_client, queue_url, batch, batch_idx - 1
        )
...
Examples:
  ...
  python push_dlq_to_sqs.py --queue-url https://sqs.us-west-2.amazonaws.com/123456789/MyQueue --dlq-dir DLQ --region us-west-2
    ...
    parser.add_argument(
        '--dry-run',
        action='store_true',
        help='Preview messages without actually sending to SQS'
    )
    parser.add_argument(
        '--batch-size',
        type=int,
        default=10,
        help='Number of messages per batch (max 10, default: 10)'
    )
